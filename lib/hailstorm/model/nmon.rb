...
class Hailstorm::Model::Nmon < Hailstorm::Model::TargetHost
  ...
  def start_monitoring()
    ...
    if self.executable_pid.nil?
      Hailstorm::Support::SSH.start(*ssh_connection_spec) do |ssh|
        ...
        command = sprintf('%s -F %s -s %d -t -p', self.executable_path,
    ...
    File.open(log_file_path, 'r') do |file|
      file.each_line do |line|
        ...
        if cpu_rexp.match(line) and cpu_samples_count < samples_count
          ...
          cpu_tokens = line.split(comma_rexp).slice(2, 2).collect {|e| e.to_f}
  ...
  def average_cpu_usage(*args)
    ...
    if args.length == 1 and args.first.is_a?(Float)
