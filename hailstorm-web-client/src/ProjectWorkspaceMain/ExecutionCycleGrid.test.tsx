...
import { CheckedExecutionCycle, ButtonStateLookup } from './ControlPanel';
...
describe('<ExecutionCycleGrid />', () => {
  ...
  it('should not have the current cycle as checkable or deletable', (done) => {
    ...
    const apiSpy = jest.spyOn(ExecutionCycleService.prototype, 'list').mockResolvedValue([
      {
        id: 1, projectId: 11, startedAt: new Date(), threadsCount: 10
      }
    ]);
    ...
    setTimeout(() => {
      ...
      component!.update();
      expect(component).toContainMatchingElements(2, 'tr');
      expect(component!.find('tr').at(1)).not.toContainMatchingElement('input[type="checkbox"]');
      expect(component!.find('tr').at(1)).not.toContainMatchingElement('a.is-danger');
    }, 0);
