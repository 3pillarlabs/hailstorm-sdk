...
export function StepHeader({ state, setDisableAbort, dispatch, handleFileUpload, setUploadAborted, uploadAborted }: {
  ...
  return (<div className={`columns ${styles.stepHeader}`}>
    ...
    <div className="column is-2">
      ...
      }} onFileUpload={handleFileUpload} onUploadError={(file, error) => {
        dispatch(new AbortJMeterFileUploadAction({ name: file.name, uploadError: error }));
        setUploadAborted(false);
        setDisableAbort(true);
      }} disabled={isUploadInProgress(state.wizardState!.activeJMeterFile)} abort={uploadAborted} pathPrefix={state.activeProject!.id.toString()}>
        <button className="button is-link is-medium is-pulled-right" title="Upload .jmx and data files (like .csv)" disabled={isUploadInProgress(state.wizardState!.activeJMeterFile)}>
          Upload
          </button>
      </FileUpload>
    </div>
