...
class Hailstorm::Model::AmazonCloud < ActiveRecord::Base
  ...
  def cleanup
    ...
    key_pair = ec2.find_key_pair(name: self.ssh_identity)
  ...
  module AgentHelper
    ...
    def start_agent(load_agent)
      ...
      agent_ec2_instance = ec2.find_instance(instance_id: load_agent.identifier) unless load_agent.identifier.nil?
    ...
    def stop_agent(load_agent)
      if load_agent.identifier
        agent_ec2_instance = ec2.find_instance(instance_id: load_agent.identifier)
    ...
    def create_agent
      ...
      security_group_ids = self.security_group.split(/\s*,\s*/)
                               .map { |x| ec2.find_security_group(name: x) }
                               .map(&:id)
  ...
  def ec2(refresh = false)
    @ec2 = nil if refresh
    @ec2 ||= aws_adapter(aws_config.merge(region: self.region, vpc_subnet_id: self.vpc_subnet_id))
  end

  # @return [Hailstorm::Support::AwsAdapter::EC2]
  def aws_adapter(attrs)
  ...
  def set_availability_zone
    ...
    self.zone = ec2.first_available_zone
  end
  ...
  module IdentityHelper
    ...
    def create_key_pair
      ...
      key_pair = ec2.create_key_pair(name: self.ssh_identity)
  ...
  module InstanceHelper
    ...
    def create_ec2_instance(attrs)
      instance = ec2.create_instance(attrs)
  ...
  module AmiHelper
    ...
    def create_agent_ami
      ...
      clean_instance = ec2.create_instance(
        new_ec2_instance_attrs(base_ami, [ec2.find_security_group(name: self.security_group).id])
      )
    ...
    def check_for_existing_ami
      ...
      ex_ami = ec2.find_self_owned_ami(regexp: regexp)
      if ex_ami
    ...
    # @param [Hailstorm::Support::AwsAdapter::EC2::Instance] instance instance that will be registered as an AMI
    # @return [String] AMI Id of the just registered AMI
    def register_hailstorm_ami(instance)
      new_ami = ec2.register_ami(
        name: ami_id,
  ...
  module VpcHelper
    def assign_vpc_subnet
      hailstorm_subnet = ec2.find_subnet(name_tag: Defaults::SUBNET_NAME) || create_hailstorm_subnet
      self.vpc_subnet_id = hailstorm_subnet.subnet_id
      ec2.vpc_subnet_id = self.vpc_subnet_id
    end
