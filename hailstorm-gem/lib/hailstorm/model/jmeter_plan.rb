      ...
      jmeter_document do |doc|
        doc.xpath('//ThreadGroup[@enabled="true"]').each do |tg|
          ...
          tg_comment_node = doc.xpath("//ThreadGroup[@testname=\"#{tg_name}\"]/stringProp[@name=\"TestPlan.comments\"]")
                               .first
          definition.thread_group = if tg_comment_node.nil?
                                      tg_name.to_s
                                    else
                                      "#{tg_name}: #{tg_comment_node.content}"
                                    end
          ...
          doc.xpath(steps_xpath_expr).each do |step| # (Sampler element)
            ...
            current_step_xpath_expr = format(tg_xpath_expr_fmt, tg_name) +
            ...
            definition.samplers << if step_comment_node.nil?
                                     step_name.to_s
                                   else
                                     "#{step_name}: #{step_comment_node.content}"
                                   end
          end
